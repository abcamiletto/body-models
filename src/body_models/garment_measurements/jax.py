...
class GarmentMeasurements(BodyModel):
    ...
    def get_rest_pose(
        ...
        global_ref = jnp.zeros((*batch_dims,), dtype=dtype)
        ...
        if hands != "default":
            axis_angle = jnp.asarray(GARMENT_HAND_PRESETS[hands], dtype=dtype).reshape(-1, 3)
        ...
        return {
            "shape": jnp.zeros((*batch_dims, self.num_shape_components), dtype=dtype),
            "body_pose": body_pose,
            "head_pose": head_pose,
            "hand_pose": hand_pose,
    ...
    def get_tpose(
        ...
        **kwargs,
        ...
        axis_angle = jnp.asarray(GARMENT_BODY_PRESETS["t_pose"], dtype=params["body_pose"].dtype)
        axis_angle = jnp.broadcast_to(axis_angle, (*batch_dims, *axis_angle.shape))
    ...
    def get_ipose(
        ...
        **kwargs,
        ...
        params = self.get_rest_pose(batch_dims=batch_dims, hands=hands, **kwargs)
        axis_angle = jnp.asarray(GARMENT_BODY_PRESETS["i_pose"], dtype=params["body_pose"].dtype)
        axis_angle = jnp.broadcast_to(axis_angle, (*batch_dims, *axis_angle.shape))
        params["body_pose"] = SO3.convert(axis_angle, src="axis_angle", dst=self.rotation_type, xp=jnp)
        return params
