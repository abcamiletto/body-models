...
class SMPLModel(SkinnedModel):
    ...
    def bind(self, shape: Float[Array, "*batch 10"]) -> BoundSMPL:
        """Prepare a reusable SMPL identity."""
        return BoundSMPL(self, self._prepare_identity(shape))
...
class BoundSMPL(BoundModel[SMPLModel, core.SmplIdentity]):
    """SMPL with shape-dependent state prepared."""
    ...
    def forward_vertices(
        ...
        """Compute posed mesh vertices."""
        model = self.model
        pose = self.prepare_pose(body_pose, pelvis_rotation)
    ...
    def prepare_pose(
        ...
        return self.model._prepare_pose(body_pose, pelvis_rotation, identity=self._identity)


__all__ = ["BoundSMPL", "SMPLModel", "SmplConfig"]
