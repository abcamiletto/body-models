...
class SMPLX(SkinnedModel, nn.Module):
    ...
    def get_rest_pose(
        ...
        head_pose_ref = torch.zeros((*batch_dims, self.NUM_HEAD_JOINTS, 3), device=device, dtype=dtype)
        ...
        params = {
            ...
            "expression": torch.zeros((*batch_dims, 10), device=device, dtype=dtype),
...
def _get_kernel(kernel: Literal["torch", "warp"]):
    ...
    try:
        from body_models.bodies.smplx.backends import warp as warp_backend
