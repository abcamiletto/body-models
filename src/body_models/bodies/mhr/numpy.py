...
class MHR(SkinnedModel):
    ...
    def get_rest_pose(
        ...
        return {
            ...
            "head_pose": np.zeros((*batch_dims, self.head_pose_dim), dtype=dtype),
            "hand_pose": hand_pose,
    ...
    def get_tpose(
        ...
        **kwargs,
        ...
        pose = np.zeros((*batch_dims, self.pose_dim), dtype=params["body_pose"].dtype)
        pose[..., :100] = np.asarray(MHR_BODY_PRESETS["t_pose"], dtype=params["body_pose"].dtype)
        params["body_pose"], params["head_pose"], _hand_pose = unpack_pose(np, pose)
        return params
