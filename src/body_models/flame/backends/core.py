...
class FlamePreparedPose(TypedDict):
    """Pose-dependent FLAME state returned by ``prepare_pose``."""
...
def prepare_pose(
    ...
    *,
    ...
    """Precompute pose-dependent FLAME state for repeated forward passes."""
    if xp is None:
    ...
    prepared_pose: FlamePreparedPose = {"joint_transforms": T_world}
    if skip_vertices:
