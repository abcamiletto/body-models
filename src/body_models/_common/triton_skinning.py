...
def _build_transform_gradient_plan(joint_indices: Tensor, joint_weights: Tensor) -> _JointMajorCsrPlan:
    ...
    if valid_positions.numel() == 0:
        empty_indices = torch.empty(0, dtype=torch.int32, device=joint_indices.device)
        empty_weights = torch.empty(0, dtype=joint_weights.dtype, device=joint_weights.device)
        offsets = torch.zeros(1, dtype=torch.int32, device=joint_indices.device)
        return _JointMajorCsrPlan(empty_indices, empty_weights, offsets)
