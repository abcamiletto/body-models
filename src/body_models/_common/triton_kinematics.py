...
def compose_parent_tree(
    ...
    if not local.is_cuda:
        raise TypeError("Triton kinematics requires CUDA tensors")
    ...
    if len(parents) != local.shape[-3]:
        raise ValueError("parents must contain one entry per joint")
    if any(parent > joint for joint, parent in enumerate(parents)):
    ...
    normalized_parents = [joint if parent < 0 else parent for joint, parent in enumerate(parents)]
    # Inductor lifts this static table into the compiled graph's constant state.
    parent_tensor = torch.tensor(normalized_parents, dtype=torch.int32, device=local.device)
    flat_local = local.reshape(-1, num_joints, 4, 4).contiguous()
    world = _compose_parent_tree_op(flat_local, parent_tensor)
