...
class MHR(BodyModel, nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.bind_inv_linear = nn.Buffer(
            SO3.conversions.from_quat_to_rotmat(q, convention="xyzw") * s.unsqueeze(-1)
        )
    ...
    def _apply_skinning(
        ...
        if global_rotation is not None:
            R = SO3.conversions.from_axis_angle_to_rotmat(global_rotation, xp=torch)
            verts = verts @ R.mT
