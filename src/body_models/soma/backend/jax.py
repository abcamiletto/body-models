...
def prepare_data(weights: SomaWeights) -> SomaWeights:
    source_topology = weights.topology
    source_correctives = weights.correctives
    vertex_map = None
    if weights.vertex_map is not None:
        vertex_map = jnp.asarray(weights.vertex_map)

    topology = replace(
        source_topology,
        joint_children_indices_full=jnp.asarray(source_topology.joint_children_indices_full),
        skinned_vertex_indices_full_index=jnp.asarray(source_topology.skinned_vertex_indices_full_index),
    )
    correctives = replace(
        source_correctives,
        corrective_bindpose=jnp.asarray(source_correctives.corrective_bindpose),
        corrective_W1=jnp.asarray(source_correctives.corrective_W1),
        corrective_W2_rows=jnp.asarray(source_correctives.corrective_W2_rows),
        corrective_W2_cols=jnp.asarray(source_correctives.corrective_W2_cols),
        corrective_W2_values=jnp.asarray(source_correctives.corrective_W2_values),
    )
    return replace(
        weights,
