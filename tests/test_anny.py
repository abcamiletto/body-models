...
def test_forward_arbitrary_batch_dims_torch(batch_shape: tuple[int, ...]) -> None:
    ...
    for key in ["gender", "age", "muscle", "weight", "height", "proportions"]:
        params_flat[key] = torch.rand(flat_batch, dtype=torch.float32)
    params_flat["pose"] = torch.randn(flat_batch, model.num_joints, 3, dtype=torch.float32) * 0.1
...
def test_forward_accelerator_optional_defaults() -> None:
    ...
    if not MODEL_PATH.exists():
        pytest.skip(f"ANNY model not found at {MODEL_PATH}")

    model = ANNY(model_path=MODEL_PATH).to(device)
    B = 2
    params = model.get_rest_pose(batch_size=B)
    params["pose"] = torch.randn(B, model.num_joints, 3, device=device, dtype=torch.float32) * 0.1
