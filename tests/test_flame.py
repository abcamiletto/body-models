...
def test_forward_arbitrary_batch_dims_torch(batch_shape: tuple[int, ...]) -> None:
    ...
    params_flat["shape"] = torch.randn(flat_batch, 300, dtype=torch.float32)
    params_flat["expression"] = torch.randn(flat_batch, 100, dtype=torch.float32) * 0.1
    params_flat["pose"] = torch.randn(flat_batch, model.NUM_HEAD_JOINTS, 3, dtype=torch.float32) * 0.1
    params_flat["head_rotation"] = torch.randn(flat_batch, 3, dtype=torch.float32) * 0.1
