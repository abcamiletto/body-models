...
def test_triton_kinematics_matches_torch_under_compile(num_joints, batch_size, multiple_roots) -> None:
    ...
    expected = kinematics.compose_kinematic_fronts(affine(reference_upper), fronts, xp=torch)
    forward = torch.compile(
        lambda value: triton_kinematics.compose_parent_tree(affine(value), parents),
        fullgraph=True,
    )
