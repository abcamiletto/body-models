...
ASSET_DIR = Path(__file__).parents[1] / "assets" / "models_hub" / "smplx-neutral"
REFERENCE_DIR = Path(__file__).parents[1] / "assets" / "references" / "smplx-neutral"
...
def test_torch_warp_kernel_matches_torch() -> None:
    ...
    model = SMPLX(model_path=MODEL_PATH, kernel="warp")
    reference_model = SMPLX(model_path=MODEL_PATH)
