...
def test_soma_shape_argument_remains_backward_compatible(
    soma_model_path: Path,
    upstream_data_root: Path,
) -> None:
    from body_models.soma.torch import SOMA

    upstream = _make_upstream_layer("soma", upstream_data_root)
    inputs = _sample_inputs(
        upstream.identity_model.num_identity_coeffs,
        upstream.identity_model.num_scale_params,
        seed=1234,
    ...
    model = SOMA(model_path=soma_model_path, model_type="soma")

    with torch.no_grad():
        verts_from_identity = model.forward_vertices(
            pose=inputs["pose"],
            global_translation=inputs["global_translation"],
            identity=inputs["identity"],
        )
        verts_from_shape = model.forward_vertices(
            shape=inputs["identity"],
            pose=inputs["pose"],
            global_translation=inputs["global_translation"],
        )
        joints_from_identity = _joint_positions(
        ...
        joints_from_shape = _joint_positions(
            model.forward_skeleton(
                shape=inputs["identity"],
                pose=inputs["pose"],
                global_translation=inputs["global_translation"],
    ...
    torch.testing.assert_close(verts_from_identity, verts_from_shape, rtol=RTOL, atol=ATOL)
    torch.testing.assert_close(joints_from_identity, joints_from_shape, rtol=RTOL, atol=ATOL)
    torch.testing.assert_close(verts_from_identity, reference["vertices"], rtol=RTOL, atol=ATOL)
    torch.testing.assert_close(joints_from_identity, reference["joints"], rtol=RTOL, atol=ATOL)
