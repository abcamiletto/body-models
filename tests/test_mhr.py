...
def test_forward_arbitrary_batch_dims_torch(batch_shape: tuple[int, ...]) -> None:
    ...
    params_flat["shape"] = torch.randn(flat_batch, model.SHAPE_DIM, dtype=torch.float32)
    params_flat["pose"] = torch.randn(flat_batch, model.pose_dim, dtype=torch.float32) * 0.05
    params_flat["expression"] = torch.randn(flat_batch, model.EXPR_DIM, dtype=torch.float32) * 0.1
